    ...
    <>
      ...
      <nav className='navbar w-full fixed top-0 flex justify-between items-center px-4 sm:px-6 py-3 text-gray-800 font-bold z-[50] bg-gradient-to-b from-white/90 to-gray-50/80 backdrop-blur-md shadow-sm h-20'>
        ...
        <div className='left-section flex-shrink-0'>
          <NavLink
            to={
              isLoggedIn ? (userRole === "admin" ? "/admin" : "/main") : "/main"
        ...
        <div className='mid-section flex-grow hidden md:flex justify-center items-center'>
          {!shouldHideNavbar(location.pathname) && !isAdminPage && (
        ...
        <div className='right-section flex items-center gap-3 sm:gap-4'>
          ...
          <div className='md:hidden'>
            {isAdminPage ? (
              <button
                onClick={() =>
                  setIsAdminMobileSidebarOpen(!isAdminMobileSidebarOpen)
                }
                aria-label='Toggle Admin Navigation'
                className='text-2xl text-gray-700 focus:outline-none hover:text-amber-500'>
                <FontAwesomeIcon
                  icon={isAdminMobileSidebarOpen ? faXmark : faBars}
                />
              </button>
            ) : (
              !updatedArray.includes(location.pathname) && (
                <button
                  ...
                  className='text-2xl text-gray-700 focus:outline-none hover:text-amber-500'>
                  <FontAwesomeIcon
                    icon={mainMobileMenuOpen ? faXmark : faBars}
                  />
                </button>
              )
            )}
        ...
        <>
          ...
          <div
            className={`fixed top-0 left-0 h-full w-3/4 max-w-xs bg-white rounded-tr-4xl rounded-br-4xl shadow-xl transform transition-transform duration-300 ease-in-out z-[1002] md:hidden ${
              isAdminMobileSidebarOpen ? "translate-x-0" : "-translate-x-full"
            }`}>
            {/* Kontainer scroll */}
              ...
              <ul className='w-full flex flex-col gap-3 text-sm flex-grow'>
                {[
                  ...
                  <li
                    ...
                    onClick={() => handleAdminNavClick(item.tab)}>
                    {item.icon && (
                      <img
                        src={item.icon}
                        alt={item.label}
        ...
        <>
          ...
          <div
            className={`fixed top-0 left-0 h-full w-3/4 max-w-xs bg-white shadow-xl  rounded-tr-4xl rounded-br-4xl transform transition-transform duration-300 ease-in-out z-[1002] p-2 md:hidden ${
              mainMobileMenuOpen ? "translate-x-0" : "-translate-x-full"
            }`}>
